...
in
{
  ...
  home-manager.users.${config.var.username}.wayland.windowManager.sway = {
    enable = true;
    package = pkgs.sway;
    ...
    config = {
      ...
      output."*" = {
        #bg = "${pkgs.swaybg}/share/backgrounds/sway/Sway_Wallpaper_Blue_1920x1080.png fill";
      };
    ...
    ########################################################################
    ## Anything not covered by first‑class options
    ########################################################################
    extraConfig = ''
      # media / volume
